...
// One service's contribution to the People rollup: the rows of its cached `members` dataset plus when
// that report was fetched.
...
const membersDataset = (result: CapabilityResult | null): TableDataset | null =>
  result?.datasets.find(
    (d): d is TableDataset => d.shape === 'table' && d.id === 'members' && d.columns.some((c) => c.key === 'email')
  ) ?? null

// Read every installed plugin's cached reports and merge their member rosters into the People payload.
// One roster per service: the capability named 'members' is the roster of record when several capabilities
// carry a matching dataset. Cache-only IO glue over the tested merge — no collectors run.
export const buildPeople = async (): Promise<PeopleDto> => {
  ...
  for (const plugin of plugins.filter((p) => getPluginInstalled(p.meta.id))) {
    const caps = [...plugin.capabilities].sort((a, b) => Number(b.id === 'members') - Number(a.id === 'members'))

    for (const cap of caps) {
      const envelope = await readCurrent(plugin.meta.id, cap.id)
      const ds = envelope ? membersDataset(reconstructResult(envelope.data)) : null

      if (ds) {
        inputs.push({
          pluginId: plugin.meta.id,
          serviceName: plugin.meta.name,
          asOf: envelope?.lastRunAt,
          rows: ds.rows
        })
        break
      }

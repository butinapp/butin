...
// `resolveSampleConfig` turns the size knobs into a concrete `SampleConfig`. `validateSamples`/`resultValidator`
// are the contract checks a plugin's tests assert with. Kept off `/util` (which is the runtime edge-normalizer
// surface) because this is demo/seed tooling, not request-path code.

export { createSampleGen, resolveSampleConfig } from './synthetic.js'
export { resultValidator, validateSamples } from './validate.js'
export type {
  SamplePerson,
  SampleConfig,
  SampleSize,
